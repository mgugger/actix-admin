...
mod webdriver_tests {
    ...
    async fn webdriver_create() -> Result<(), fantoccini::error::CmdError> {
        ...
        assert!(url
            .as_ref()
            .contains("http://localhost:5555/admin/post/list"));
        ...
        assert!(
            html_source.contains("testtitle"),
            "Expected testtitle in the table"
        );
        assert!(
            html_source.contains("testtext"),
            "Expected testtext in the table"
        );
        assert!(
            html_source.contains("2024-04-02"),
            "Expected correct date in the table"
        );
        assert!(
            html_source.contains("EverydayTea"),
            "Expected EverydayTea in the table"
        );
